 ...
 */
function Cancel_Button({ onNext, classification = 'Malicious', tooltipText = "Override the AI's recommendation" }) {
  const [showTooltip, setShowTooltip] = useState(false);
  const [showModal, setShowModal] = useState(false);

  /**
   * Handles the button click event
   * Shows the success modal when the override button is clicked
   */
  const handleClick = () => {
    setShowModal(true);
  };
